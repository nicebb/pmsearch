...
class ucp_pmsearch_module
{
	...
	{
		/* @var $phpbb_container \Symfony\Component\DependencyInjection\ContainerInterface */
		...
		/* @var $path_helper \phpbb\path_helper */
		...
		$folder = get_folder($user->data['user_id']);

		// Get the query so we'll have something to do
		$keywords = utf8_normalize_nfc(trim($request->variable('keywords', '', true)));

		// If the user don't have JavaScript on, we need to know if the advanced search is activated
		$advanced_search = $request->variable('pmsearch', '');

		// We try to guess the folder to search by inspecting the referer
		$default_folder = array(PRIVMSGS_INBOX);
		$parts = $path_helper->get_url_parts($request->server('HTTP_REFERER', ''));
		if (isset($parts['params']['folder']))
		{
			$value = $parts['params']['folder'];
			if (in_array($value, array('inbox', 'outbox', 'sentbox')))
			{
				$value = ($value == 'inbox') ? PRIVMSGS_INBOX : (($value == 'outbox') ? PRIVMSGS_OUTBOX : PRIVMSGS_SENTBOX);
			}
			$default_folder = array($value);
		}
		// Get the search-keys
		$search_exact = $request->variable('exact', 0); // Exact phrase match
		$search_case = $request->variable('case', 0); // Case sensitive searches
		$search_folder = $request->variable('folders', array(0)); // Search in these folders
		if (empty($search_folder))
		{
			// We do not use $default_folder as a default value in $request->variable to make difference between array(0) and array(PRIVMSGS_INBOX)
			$search_folder = $default_folder;
		}

		$folder_id = (count($search_folder) === 1) ? current($search_folder) : null;

		foreach ($search_folder as $f_id)
		{
			$search_folder_names[] = $folder[$f_id]['folder_name'];
			$folder[$f_id]['search_enabled'] = true;
		}
		foreach ($folder as $f_id => $folder_ary)
		{
			$template->assign_block_vars('folder_options', array(
				'FOLDER_ID' => $f_id,
				'FOLDER_NAME' => $folder_ary['folder_name'],
				'SEARCH_ENABLED' => $folder_ary['search_enabled'],
			));
		}

		$search_author = $request->variable('author', 0); // Search for sender, recipient, both or message content
		$search_all_words = $request->variable('all_words', 0); // Search for all words. Equals a + sign in front of every word
		$search_word = $request->variable('word', 0); // Search only for whole words. Not partial words
		$search_within = $request->variable('within', PM_BOTH); // Search in subject, message or both.
		...
		if ($sort_days)
		{
			...
			if ($request->variable('sort', false))
			{
				$start = 0;
			}
		}
		...
		$search_folder_sql = ($search_folder) ? ' AND ' . $db->sql_in_set('t.folder_id', $search_folder) : 'AND t.folder_id != ' . PRIVMSGS_HOLD_BOX . ' AND t.folder_id != ' . PRIVMSGS_NO_BOX;
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if (($pm_count > $start && $pm_count <= $page_end) || $sort_by_author)
			{
				...
				// Generate the URLs...
				$view_message_url = append_sid($phpbb_root_path . 'ucp.' . $phpEx, 'i=pm&amp;mode=view&amp;p=' . $row_message_id . $hilit);
				$remove_message_url = append_sid($phpbb_root_path . 'ucp.' . $phpEx, 'i=pm&amp;mode=compose&amp;action=delete&amp;p=' . $row_message_id);
				...
				$messagerow = array(
					...
					'FOLDER' => $folder[$row['folder_id']]['folder_name'],
		...
		$pagination_params = "&amp;$u_sort_param&amp;exact=$search_exact&amp;case=$search_case&amp;author=$search_author&amp;keywords=$keywords";
		foreach ($search_folder as $f_id)
		{
			$pagination_params .= '&amp;folders[]=' . $f_id;
		}
		$pagination->generate_template_pagination(
				append_sid($this->u_action . $pagination_params), 'pagination', 'start', $pm_count, $config['topics_per_page'], $start);

		// Send vars to template
		$template->assign_vars(array(
			'S_PRIVMSGS' => true,
			'S_SHOW_COLOUR_LEGEND' => true,
			'S_PMSEARCH_ACTION' => $this->u_action,
			'S_PM_ACTION' => append_sid("{$phpbb_root_path}ucp.$phpEx", "i=pm&amp;mode=view&amp;action=view_folder&amp;f=$folder_id" . (($start !== 0) ? "&amp;start=$start" : '')),
			'SEARCH_FOLDER_NAMES' => implode(', ', $search_folder_names),
